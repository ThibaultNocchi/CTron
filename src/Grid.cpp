...
Grid::Grid(const COORDTYPE width, const COORDTYPE height) {
  ...
  this->indexes.resize(this->getHeight());
  for (COORDTYPE x = 0; x < this->getHeight(); ++x) {
    this->indexes[x].resize(this->getWidth());
    for (COORDTYPE y = 0; y < this->getWidth(); ++y) {
        this->indexes[x][y] = x * this->getWidth() + y;
    }
  }
...
int Grid::moveSnakes() {
  ...
  for (size_t i = 0; i < this->snakes.size(); ++i) {
    ...
    if (this->snakes[i].getAlive()) {
      ...
      if (this->snakes[i].getCurrentLength() ==
          this->snakes[i].getAdultLength()) {
  ...
  // First checking for collisions with a wall or a snake (excepted the future
  // head) Then checking for frontal collisions with another future head.
  for (size_t i = 0; i < this->snakes.size(); ++i) {
      ...
      if (cellValue == SNAKE || cellValue == WALL || cellValue == HEAD) {
        ...
        for (size_t j = (i + 1); j < this->snakes.size(); ++j) {
          if (this->snakes[j].getAlive()) {
            ...
            if (newHead.first == newHead2.first &&
                newHead.second == newHead2.second) {
              this->killSnake(i);
              this->killSnake(j);
            }
          }
  ...
  int leftAlive = 0;
  size_t winnerId; // Updated on each living snake, but it is only returned if
                   // there is a single living snake.
...
void Grid::applySnakeHash(const COORDTYPE x, const COORDTYPE y,
                          const size_t snakeID, const State s) {
  if (s == HEAD) {
    this->currentHash ^=
        this->snakesHashes[this->getIndex(x, y)][snakeID].first;
  } else {
    this->currentHash ^=
        this->snakesHashes[this->getIndex(x, y)][snakeID].second;
  }
}
...
void Grid::displayGridBasic() {
  ...
  for (COORDTYPE x = 0; x < this->getHeight(); ++x) {
    for (COORDTYPE y = 0; y < this->getWidth(); ++y) {
      ...
      case HEAD:
        if (this->snakes[0].getHead().first == x &&
            this->snakes[0].getHead().second == y)
          std::cout << "T";
        else
          std::cout << "H";
        break;
...
COORDTYPE Grid::getWidth() const { return this->width; }

COORDTYPE Grid::getHeight() const { return this->height; }

COORDS Grid::getSize() const { return COORDS(this->width, this->height); }

int Grid::getIndex(const COORDTYPE x, const COORDTYPE y) const {
    return this->indexes[x][y];
  /* Utiliser directement l'attribut plutôt que la fonction get ne serait il pas
   * plus efficace ? */
}
...
const std::vector<Snake> &Grid::getSnakes() const { return this->snakes; }

int Grid::getNumberAliveSnakes() const { return this->aliveSnakes; }

const COORDS &Grid::getBonus() const { return this->bonus; }

const std::vector<State> &Grid::getCells() const { return this->cells; }

u_int64_t Grid::getHash() const { return this->currentHash; }
...
void Grid::setNewRandomDirectionsExcept(size_t index, bool noStupidMove) {
  for (size_t i = 0; i < this->snakes.size(); ++i) {
    if (i != index && this->snakes[i].getAlive())
      this->setRandomDirection(i, noStupidMove);
  }
}
...
bool Grid::operator<(const Grid &other) const {
  ...
  if (this->snakes.size() < other.getSnakes().size())
    return true;
  else if (this->snakes.size() > other.getSnakes().size())
    return false;

  auto bonus1 = this->getBonus();
  auto bonus2 = other.getBonus();

  if (bonus1.first < bonus2.first)
    return true;
  if (bonus1.first > bonus2.first)
    return false;

  if (bonus1.second < bonus2.second)
    return true;
  if (bonus1.second > bonus2.second)
    return false;
  ...
  for (size_t i = 0; i < snakes1.size(); ++i) {
    if (!snakes1[i].getAlive() && snakes2[i].getAlive())
      return true;
    else if (snakes1[i].getAlive() && !snakes2[i].getAlive())
      return false;

    if (snakes1[i].getCurrentLength() < snakes2[i].getCurrentLength())
      return true;
    if (snakes1[i].getCurrentLength() > snakes2[i].getCurrentLength())
      return false;
